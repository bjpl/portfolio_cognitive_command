...
describe('Brief Generator Skill', () => {
  ...
  // Helper function to create mock shard with health data
  function createMockShard(
    ...
    // Convert alignment score to health score (0-100 scale)
    const healthScore = Math.round(alignmentScore * 100);
    ...
    return {
      ...
      health: {
        score: healthScore,
        grade: healthGrade as 'A' | 'B' | 'C' | 'D' | 'F',
        factors: {
          activity: Math.min(100, healthScore + 10),
          deployment: healthScore,
          codeQuality: Math.max(0, healthScore - 5),
          documentation: Math.max(0, healthScore - 10),
          integrations: healthScore
        },
        recommendations: healthScore < 70 ? ['Add tests', 'Improve documentation'] : []
      }
    };
  }
  ...
  describe('analyzePortfolio', () => {
    it('should identify red list projects (low alignment and drift)', () => {
      const shards: ProjectShard[] = [
        ...
        createMockShard('low-alignment', 'Infra', 0.45, 'ACTIVE', false)
      ];
  ...
  describe('generateBriefMarkdown', () => {
    it('should generate valid markdown with all sections', () => {
      ...
      // Check key sections (updated for health-based scoring)
      expect(markdown).toContain('# Portfolio Brief');
      expect(markdown).toContain('## 🏥 Portfolio Health Score');
      expect(markdown).toContain('## 📊 Executive Summary');
      expect(markdown).toContain('## 🚨 Projects Needing Attention');
    ...
    it('should display health grade', () => {
      ...
      // Health-based output shows grade
      expect(markdown).toContain('Grade:');
    });

    it('should show "all projects in good health" when no projects need attention', () => {
      const shards: ProjectShard[] = [
        createMockShard('project-a', 'Experience', 0.92, 'ACTIVE'),
      ...
      // Health-based output shows good health message
      expect(markdown).toContain('✅');
      expect(markdown).toContain('good health');
    });

    it('should list projects needing attention', () => {
      const shards: ProjectShard[] = [
      ...
      // Health-based output shows projects in table
      expect(markdown).toContain('drift-project');
    ...
    it('should generate recommendations for dormant projects', () => {
      ...
      // More than 50% dormant triggers recommendation
      const metrics = createMockMetrics(2, 0, 0);
      metrics.dormantProjects = 2;
      const data = analyzePortfolio(shards, metrics);
      const markdown = generateBriefMarkdown(data);

      // Either shows dormant recommendation or healthy portfolio
      expect(markdown).toContain('Strategic Recommendations');
    });

    it('should show health grade when all metrics are good', () => {
      const shards: ProjectShard[] = [
        createMockShard('project-a', 'Experience', 0.92, 'ACTIVE'),
        createMockShard('project-b', 'Core Systems', 0.88, 'ACTIVE')
      ];
  ...
  describe('Edge cases and error handling', () => {
    ...
    it('should handle long intent text in markdown', () => {
      ...
      // Should contain project name in output
    ...
    it('should handle projects with same alignment score', () => {
      ...
      // With health >= 70 and ACTIVE status, should be in top performers
      // Health score is 90 (from 0.9 alignment * 100)
      expect(data.topPerformers.length).toBe(3);
    });
  });
});
